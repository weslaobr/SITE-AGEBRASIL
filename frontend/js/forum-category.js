// forum-category.js - VERSÃO CORRIGIDA COM AVATARES
class ForumCategoryUI {
    ...
    setupAuthListener() {
        ...
        document.addEventListener('authStateChanged', (event) => {
            console.log('🔄 Auth state changed, atualizando UI...');
            this.checkAuthState();
    ...
    async loadTopics() {
        try {
            ...
            // 🔥 CORREÇÃO: Padronizar campos e garantir avatar
            topics = topics.map(t => ({
                ...t,
            ...
            console.log('📋 Tópicos carregados:', topics.map(t => ({
                id: t.id,
                title: t.title,
                author: t.author,
                authorId: t.authorId,
                authorAvatar: t.authorAvatar,
                hasAvatar: !!t.authorAvatar
            })));
            ...
            const topicsHTML = await Promise.all(topics.map(async (topic) => {
                ...
                // 🔥 CORREÇÃO: Gerar URL do avatar corretamente
                const avatarUrl = this.getAvatarUrl(topic.authorId, topic.authorAvatar);
                console.log('🖼️ Avatar URL para', topic.author, ':', avatarUrl);
    ...
    // 🔥 NOVO MÉTODO: Gerar URL do avatar corretamente
    getAvatarUrl(userId, avatarHash) {
        ...
        if (avatarHash) {
            return `https://cdn.discordapp.com/avatars/${userId}/${avatarHash}.webp?size=45`;
        } else {
